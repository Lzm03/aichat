...
try{
  ...
  const r5=await runLoad('ask_url_mock',40,120,async(i)=>{const r=await fetch('http://127.0.0.1:4000/api/ask-url',{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${token}`},body:JSON.stringify({systemPrompt:'总结',url:'https://example.com',modelProvider:'gemini'})}); if(!r.ok) throw 1; await r.arrayBuffer();});
