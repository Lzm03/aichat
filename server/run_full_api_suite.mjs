...
const serverEnv = { ...process.env, PORT: loadTestPort, MOCK_UPSTREAM: 'true', MOCK_UPSTREAM_MIN_DELAY_MS: '300', MOCK_UPSTREAM_MAX_DELAY_MS: '2000', MOCK_UPSTREAM_FAILURE_RATE: '0.02' };
...
try{
  ...
  const loginOnce = await fetch(`${loadTestBaseUrl}/api/auth/login`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email,password:pwd})});
  const token = (await loginOnce.json()).token;

  const r1=await runLoad('auth_login',100,1000,async()=>{const r=await fetch(`${loadTestBaseUrl}/api/auth/login`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email,password:pwd})}); if(!r.ok) throw 1; await r.arrayBuffer();});
  const r2=await runLoad('auth_me',100,1500,async()=>{const r=await fetch(`${loadTestBaseUrl}/api/auth/me`,{headers:{authorization:`Bearer ${token}`}}); if(!r.ok) throw 1; await r.arrayBuffer();});
  const r3=await runLoad('ask_mock',100,600,async(i)=>{const r=await fetch(`${loadTestBaseUrl}/api/ask`,{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${token}`},body:JSON.stringify({systemPrompt:'你是老师',userPrompt:`Q${i}`,stream:false,usageType:'general',botId:'default',modelProvider:'gemini'})}); if(!r.ok) throw 1; await r.arrayBuffer();});
  const r4=await runLoad('tts_mock',30,120,async(i)=>{const r=await fetch(`${loadTestBaseUrl}/api/tts`,{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${token}`},body:JSON.stringify({text:`hello ${i}`,voiceId:'mock-voice-1',usageType:'chat_voice'})}); if(!r.ok) throw 1; await r.arrayBuffer();});
  const r5=await runLoad('ask_url_mock',40,120,async(i)=>{const r=await fetch(`${loadTestBaseUrl}/api/ask-url`,{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${token}`},body:JSON.stringify({systemPrompt:'总结',url:'https://example.com',modelProvider:'gemini'})}); if(!r.ok) throw 1; await r.arrayBuffer();});
