...
export async function getOrCreateWebmSequence(
  ...
  const folderUrl = `${publicBase}/uploads/sequences/${id}/frames`;
...
router.post("/webm-sequence", async (req, res) => {
  try {
    ...
    try {
      manifest = await getOrCreateWebmSequence(videoUrl, fps, base);
    } catch (e) {
    ...
    console.error("webm-sequence failed", err);
    return res.status(500).json({ error: "webm-sequence failed" });
  }
});
