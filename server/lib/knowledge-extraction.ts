...
2. 再生成核心知識點，數量按教材篇幅、概念密度同結構動態決定：
- 先睇結構：如果原文本身有明顯章節／標題／多個單元結構，按資料自身結構判斷數量——每個章節/單元抽出真正需要掌握嘅核心概念；字數門檻只係無結構資料嘅預設
- 無明顯結構時按字數分兩級：短篇（原文約 1000 字或以內，如一頁課文）→ 3 到 5 個；長篇（超過 1000 字）→ 6 到 20 個，通常 10 個以內，跨單元先按密度加多
- 硬上限：總數最多 20 個，而且每個 tier 最多 10 個——學生一次過見太多知識點會產生畏懼心理，寧精勿多
