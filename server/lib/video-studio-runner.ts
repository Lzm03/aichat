...
const SLOT_PROMPTS: Record<VideoStudioSlotKey, string> = {
  ...
  speaking:
    "Speaking mode with restrained lip-sync only. HIGHEST PRIORITY — exact source-image composition lock: use the input image as an immutable spatial layout reference. In every frame, preserve the subject's exact bounding box from the source image: identical center position, head-top position, foot position, width, height, scale, and empty margins on all four sides. The character must remain at exactly the same distance from the camera. Do not move closer to the camera, enlarge the subject, shrink the subject, reframe, recrop, or change perspective. The first frame and every following frame must overlay the source-image silhouette without any visible translation or scale change. Character stands in place with both feet planted at exactly the same position. No walking, no stepping, no body translation, and no turn-and-shift. Arms, hands, fingers, shoulders, torso, hips, legs, and posture must stay completely still. Do not gesture, wave, raise hands, move arms, shift weight, sway, lean, nod, or perform any body acting. Natural speech lip-sync is allowed only through small, controlled mouth movement. Keep mouth opening narrow and modest: no wide-open mouth, no exaggerated jaw drop, no visible teeth, no tongue, no shouting expression, and no large facial acting. Lips should move subtly as if speaking softly. Only tiny facial changes and natural blinking are allowed; keep the head completely still. Camera must be fully locked: no zoom, no push-in, no pull-back, no pan, no dolly, and no shake. Background must be pure bright chroma key green, close to RGB 0,255,0, as a single uniform solid color with no gradient, no texture, no shadow, no reflection, no noise, and no background objects. The subject must have no green spill and no green edge halo. Edges must be crisp and clean for keying, including hair strands, fingers, and clothing contours. If constraints conflict, priority order is: exact source-image composition and scale > no body movement > restrained mouth-only lip-sync > green screen purity.",
  thinking:
