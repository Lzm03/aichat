  ...
  return (
    <div className="flex flex-col items-center pt-16 text-center animate-fade-in space-y-6">
      <div className="space-y-2">
        <h3 className="text-2xl font-bold text-[#1E293B]">{uiText("為您的 AI 機器人命名")}</h3>
        <p className="text-sm text-slate-500">{uiText("一個好名字是偉大旅程的開始。")}</p>
      </div>
      
      <div className="w-full max-w-md">
        <label htmlFor="bot-name" className="sr-only">{uiText("機器人名稱")}</label>
        <input 
          type="text"
          id="bot-name"
          value={botName}
          onChange={(e) => updateConfig('name', e.target.value)}
          placeholder={uiText("例如：數學解題小能手")}
