...
export default function VideoStudioModal({
  ...
  // 动作对应的 prompts
  const prompts = {
    idle: "角色必須原地站定，雙腳固定在同一位置，禁止走動、踏步、位移、轉身移位。角色保持静止、自然呼吸、微微眨眼。相機完全固定，不縮放、不前後移動、不平移、不搖晃。只允許角色本身的輕微動作，不要移動取景框。",
    speaking: "角色必須原地站定，雙腳固定在同一位置，禁止走動、踏步、位移、轉身移位。角色自然張嘴說話，口型連貫、清晰。相機完全固定，不縮放、不推拉、不運鏡、不搖晃。保持角色在畫面中固定位置，只演示口型與表情。",
    thinking: "角色必須原地站定，雙腳固定在同一位置，禁止走動、踏步、位移、轉身移位。角色做出思考動作（抬頭、皱眉、輕微眼球運動）即可。相機固定鎖死，不前後移動、不左右平移、不縮放、不搖鏡。禁止鏡頭動畫，僅允許角色頭部小幅度動作。",
  };
