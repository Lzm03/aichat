    ...
    <div className="fixed inset-0 z-50 bg-[#F8FAFC] flex overflow-hidden">
      <div className="w-[280px] bg-white border-r border-slate-200 flex flex-col shrink-0 shadow-sm z-10">
        <div className="p-4 border-b border-slate-100">
          ...
          <h2 className="font-bold text-slate-800 leading-tight">{data?.quiz?.title || '測驗詳情'}</h2>
        </div>

        <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-1">
          {loading ? <div className="p-4 text-sm text-slate-400">正在載入學生作答...</div> : null}
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${statusClass}`}>{statusText}</span>
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50/50">
          <div className="flex justify-between text-xs font-bold text-slate-500 mb-2">
            <span>批改進度</span>
            <span>{data?.metrics?.completed || 0} / {data?.metrics?.totalStudents || 0} 人</span>
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[#F8FAFC]">
        <div className="p-6 border-b border-slate-100 bg-white flex items-center justify-between">
          <div>
            <div className="text-[25px] font-black leading-tight text-slate-900">{currentStudent?.name || '學生'}</div>
            <div className="mt-2 text-sm font-semibold text-slate-400">提交於 {currentStudent?.submittedAt ? new Date(currentStudent.submittedAt).toISOString().slice(5, 16).replace('T', ' ') : '--'}</div>
          </div>
          <div className="rounded-full bg-indigo-50 px-4 py-2 text-base font-black text-indigo-600">
            總分 {currentStudent?.score || 0} / {currentStudent?.totalPoints || 0}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {activeAnomaly ? (
            <div className="rounded-2xl border border-rose-200 bg-rose-50 p-5 text-rose-700">
              <div className="flex items-center gap-2 text-xl font-black">{activeAnomaly.title}</div>
              <div className="mt-2 text-base font-medium leading-7">{activeAnomaly.message}</div>
            </div>
          ) : null}
          ...
          {currentStudent?.answers?.map((answer: any, index: number) => (
            <div key={`${answer.questionId}-${index}`} className="rounded-[28px] border border-slate-100 bg-white p-7 shadow-sm">
              ...
              <div className={`mt-4 rounded-2xl border px-5 py-4 text-base font-bold ${
                ...
                <div>學生作答：{answer.studentAnswer || '未作答'}</div>
                <div className="mt-2">正確答案：{formatCorrectAnswer(answer)}</div>
                <div className="mt-2 text-right text-2xl font-black">{answer.score} / {answer.maxScore}</div>
              </div>
              {answer.feedback ? <div className="mt-4 rounded-2xl bg-slate-50 px-4 py-3 text-sm font-medium leading-7 text-slate-600">AI 評語：{answer.feedback}</div> : null}
            </div>
          ))}
        </div>
