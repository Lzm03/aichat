      ...
      <div className="bg-white p-4 md:p-6 rounded-[24px] shadow-[0_10px_15px_-3px_rgba(0,0,0,0.05)] flex flex-col h-full border border-slate-100">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-slate-800 flex items-center">
            <BarChart2 className="w-5 h-5 mr-2 text-indigo-500" />{uiText("評測質量總覽")}</h3>
        </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-slate-900/40 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="w-full max-w-5xl max-h-[90vh] bg-slate-50 rounded-[24px] shadow-2xl flex flex-col overflow-hidden">
              <div className="px-6 py-4 bg-white border-b border-slate-100 flex items-center justify-between shrink-0">
                <div>
                  <h2 className="text-xl font-bold text-slate-800">{selectedAssessment.title}{uiText(" - 質量分析")}</h2>
                  <p className="text-sm text-slate-500 mt-1">{uiText("發佈日期: ")}{selectedAssessment.date ? new Date(selectedAssessment.date).toISOString().slice(5, 10) : '--'}</p>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
                    <span className="text-sm font-bold text-slate-500">{uiText("本次測驗平均分")}</span>
                    <div className="text-3xl font-black text-slate-800 mt-2">{details?.metrics?.averageScore || 0}</div>
                  </div>
                  <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
                    <span className="text-sm font-bold text-slate-500">{uiText("AI 與教師分數差異")}</span>
                  ...
                  <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
                    <span className="text-sm font-bold text-slate-500">{uiText("教師修改率")}</span>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                  <div className="flex items-center justify-between p-4 border-b border-slate-100">
                    ...
                    >
                      <Download className="w-4 h-4" />{uiText("匯出 CSV")}</button>
                  </div>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm text-left text-slate-600">
                        <thead className="text-xs text-slate-500 uppercase bg-slate-50">
                          <tr>
                            <th className="px-4 py-3 font-bold">{uiText("題目 ID")}</th>
                            <th className="px-4 py-3 font-bold">{uiText("題型")}</th>
                            <th className="px-4 py-3 font-bold">{uiText("布魯姆層級")}</th>
                            <th className="px-4 py-3 font-bold text-center">{uiText("AI 分數")}</th>
                            <th className="px-4 py-3 font-bold text-center">{uiText("教師最終分數")}</th>
                            <th className="px-4 py-3 font-bold text-center cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => setSortOrder((prev) => (prev === 'desc' ? 'asc' : 'desc'))}>
                              <div className="flex items-center justify-center gap-1">{uiText("差值 ")}<ArrowUpDown className="w-3 h-3" /></div>
                            </th>
                          </tr>
                        </thead>
                        <tbody>
                          {sortedRows.map((detail) => (
                            <tr key={`${detail.id}-${detail.type}-${detail.bloom}`} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition-colors">
                              ...
                              <td className="px-4 py-3">{uiText(detail.type)}</td>
                              <td className="px-4 py-3"><span className="bg-slate-100 text-slate-600 px-2 py-1 rounded-md text-xs font-bold">{uiText(detail.bloom)}</span></td>
