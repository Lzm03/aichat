...
import { Archive, ArchiveRestore, CheckCircle2, ChevronDown, Edit3, Trash2 } from 'lucide-react';
...
export const MyQuizzesView: React.FC<MyQuizzesViewProps> = ({
  ...
  const handleArchiveToggle = async (quiz: PublishedQuizSummary, archived: boolean) => {
    setArchivingId(quiz.id);
    try {
      const response = await fetch(`${API_BASE}/api/quizzes/${quiz.id}/archive`, {
        method: 'PATCH',
      ...
      const data = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(data?.error || uiText("測驗暫時無法封存或還原，請稍後再試。"));
      }
      setPublished((prev) => prev.map((item) => (
        item.id === quiz.id
          ? { ...item, archivedAt: data?.archivedAt ?? (archived ? new Date().toISOString() : null) }
          : item
      )));
      // 封存之後張卡會離開主列表，即刻展開「已封存」區等老師睇到佢去咗邊
      if (archived) setShowArchived(true);
    } catch (error) {
      showAlert({
        title: archived ? uiText("封存失敗") : uiText("還原失敗"),
        message: error instanceof Error ? error.message : uiText("測驗暫時無法封存或還原，請稍後再試。"),
        tone: "danger",
      });
    } finally {
      setArchivingId(null);
  ...
  const activePublished = published.filter((item) => !item.archivedAt);
  const archivedPublished = published.filter((item) => Boolean(item.archivedAt));

  const renderPublishedCard = (item: PublishedQuizSummary) => {
    return (
      <motion.div
        key={item.id}
        whileHover={{ y: -4 }}
          ...
          <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
            ...
            <button
              ...
              onClick={(event) => {
                ...
                void handleArchiveToggle(item, true);
              }}
              disabled={archivingId === item.id}
              aria-label={uiTemplate("封存測驗 {0}", item.title)}
              className="w-8 h-8 shrink-0 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 hover:bg-white hover:text-indigo-600 hover:shadow-sm transition-all disabled:opacity-50"
            >
  ...
  /**
   * 已封存用緊湊單行（唔用卡片）。封存咗嘅測驗已經收起，唔應該同進行中嘅爭視覺重量；
   * 但佢保留住作答同成績紀錄，所以照樣撳得入 Drawer。
   */
  const renderArchivedRow = (item: PublishedQuizSummary) => (
    <div
      key={item.id}
      onClick={() => {
        setDetailInitialTab('results');
      ...
      className="group flex items-center justify-between gap-4 rounded-2xl border border-slate-100 bg-white p-5 shadow-sm transition-all cursor-pointer hover:border-slate-200 hover:bg-slate-50/60 hover:shadow-md"
    >
      <div className="min-w-0">
        <h3 className="truncate font-semibold text-slate-500 transition-colors group-hover:text-slate-700">{item.title}</h3>
      ...
      <div className="flex items-center gap-2 shrink-0">
        <button
          type="button"
          onClick={(event) => {
            // 成行 click 會開 Drawer，所以一定要截住
            event.stopPropagation();
            void handleArchiveToggle(item, false);
          }}
          disabled={archivingId === item.id}
          aria-label={uiTemplate("還原測驗 {0}", item.title)}
          className="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-white group-hover:text-indigo-600 group-hover:shadow-sm transition-all disabled:opacity-50"
        >
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {publishedLoading ? (
              ...
              activePublished.map((item) => renderPublishedCard(item))
            ) : published.length ? (
              <div className="col-span-full rounded-[24px] border border-dashed border-slate-200 bg-white p-10 text-center text-sm font-semibold text-slate-400">{uiText("所有測驗都已封存")}</div>
            ) : (
          ...
          {!publishedLoading && archivedPublished.length ? (
            <div className="space-y-3">
              <button
                type="button"
                onClick={() => setShowArchived((prev) => !prev)}
                aria-expanded={showArchived}
                className="flex w-full items-center gap-3 rounded-2xl border border-slate-200 bg-white px-5 py-4 text-left transition hover:bg-slate-50"
              >
                <Archive className="h-4 w-4 shrink-0 text-slate-400" aria-hidden="true" />
                <span className="text-sm font-bold text-slate-600">{uiText("已封存")}</span>
                <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-bold text-slate-500">{archivedPublished.length}</span>
                <ChevronDown className={`ml-auto h-4 w-4 shrink-0 text-slate-400 transition-transform ${showArchived ? 'rotate-180' : ''}`} aria-hidden="true" />
              </button>
              {showArchived ? (
                <div className="space-y-3">
                  <p className="px-1 text-xs font-semibold text-slate-400">{uiText("封存只會收起測驗，作答與成績紀錄都會保留，隨時可以還原。")}</p>
                  <div className="space-y-3">
                    {archivedPublished.map((item) => renderArchivedRow(item))}
                  </div>
                </div>
              ) : null}
            </div>
