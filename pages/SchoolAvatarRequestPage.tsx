...
// Request-size limit enforced by the API, not a school allowance.
const MAX_ROLES_PER_REQUEST = 10;
...
const FieldLabel: React.FC<{ children: React.ReactNode; required?: boolean; optional?: boolean }> = ({ children, required, optional }) => (
  <span className="mb-2 block text-xs font-bold text-slate-600">
    {children}{required ? <span className="ml-1 text-rose-500">*</span> : null}{optional ? <span className="ml-1 font-medium text-slate-400">{uiText("（選填）")}</span> : null}
  </span>
);
  ...
  <div className="mt-3 flex flex-wrap gap-2">
    {files.map((file, index) => (
      <span key={`${file.name}-${index}`} className="inline-flex max-w-full items-center gap-2 rounded-full bg-slate-100 py-1.5 pl-3 pr-1.5 text-xs font-bold text-slate-600">
        ...
        <button type="button" onClick={() => onRemove(index)} className="rounded-full p-1 text-slate-400 hover:bg-white hover:text-rose-500" aria-label={uiTemplate("移除 {0}", file.name)}><X className="h-3 w-3" /></button>
      </span>
    ))}
  </div>
) : null;
    ...
    <main className="min-h-screen bg-[#f7f8fb] text-slate-900">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto max-w-[928px] px-5 py-6 sm:px-8 sm:py-7">
          <a href="/" className="mb-6 inline-flex items-center gap-2 text-xs font-bold text-slate-400 transition hover:text-indigo-600"><ArrowLeft className="h-4 w-4" />{uiText(" 返回 ChopReality")}</a>
          <div className="flex items-start justify-between gap-6">
            <div>
              <motion.h1 initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="text-2xl font-black tracking-tight text-slate-950 sm:text-[28px]">{uiText("學校專屬 · AI 數字人客製化配置通道")}</motion.h1>
              <p className="mt-4 max-w-3xl text-sm leading-6 text-slate-600">{uiText("填寫使用情境與背景資料後，AI 會先生成詳細角色設定草案。老師確認後，ChopReality 團隊才會正式開始製作。")}</p>
            </div>
          ...
          <div className="mt-6 flex items-center gap-2 text-xs font-bold text-slate-400">
            {['填寫需求', 'AI 生成草案', '確認並提交'].map((step, index) => <React.Fragment key={step}><span className={index === 0 ? 'text-indigo-600' : ''}>{index + 1}. {uiText(step)}</span>{index < 2 ? <span className="h-px w-5 bg-slate-200" /> : null}</React.Fragment>)}
          </div>
        </div>
      </header>
          ...
          {reference ? (
            <motion.section key="success" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="mb-6 rounded-3xl border border-emerald-200 bg-emerald-50 p-7 sm:p-8">
              <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-start gap-4"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-emerald-600 text-white"><Check className="h-5 w-5" /></span><div><h2 className="text-lg font-black text-emerald-900">{uiText("已收到貴校的需求")}</h2><p className="mt-1 text-sm leading-6 text-emerald-800">{uiText("參考編號 ")}{reference}</p></div></div>
                <button type="button" onClick={startAnother} className="shrink-0 rounded-xl bg-emerald-600 px-5 py-3 text-sm font-black text-white transition hover:bg-emerald-700">{uiText("繼續提交下一批")}</button>
              </div>
            </motion.section>
          ) : null}
        </AnimatePresence>
        ...
        {!reference ? (
          <>
            <section className="rounded-3xl border border-slate-200 bg-white p-6 sm:p-7">
              <SectionTitle number="1" title={uiText("學校與聯絡資料")} subtitle="請填寫學校及聯絡資料。" />
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                <label className="sm:col-span-2"><FieldLabel required>{uiText("學校名稱")}</FieldLabel><input value={schoolName} onChange={(event) => setSchoolName(event.target.value)} className={inputClass} placeholder={uiText("請輸入學校全名")} /></label>
                <label><FieldLabel required>{uiText("聯絡老師")}</FieldLabel><input value={teacherName} onChange={(event) => setTeacherName(event.target.value)} className={inputClass} placeholder={uiText("老師姓名")} /></label>
                <label><FieldLabel required>{uiText("電話／WhatsApp")}</FieldLabel><input type="tel" value={phone} onChange={(event) => setPhone(event.target.value)} className={inputClass} placeholder={uiText("例如：9123 4567")} /></label>
                <label className="sm:col-span-2"><FieldLabel required>{uiText("聯絡電郵")}</FieldLabel><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} className={inputClass} placeholder="name@school.edu.hk" /></label>
            ...
            <div className="mt-7 flex items-end justify-between"><div><h2 className="text-xl font-black text-slate-900">{uiText("2. 角色需求")}</h2><p className="mt-1 text-xs text-slate-400">{uiText("每個角色均需生成並確認一份設定草案。")}</p></div><span className="text-xs font-black text-slate-400">{roles.length}{uiText(" 個")}</span></div>
            ...
            <button type="button" onClick={() => setRoles((current) => current.length < MAX_ROLES_PER_REQUEST ? [...current, createRole()] : current)} disabled={roles.length >= MAX_ROLES_PER_REQUEST} className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-indigo-200 bg-indigo-50/40 px-5 py-4 text-sm font-black text-indigo-700 transition hover:border-indigo-300 hover:bg-indigo-50 disabled:cursor-not-allowed disabled:opacity-50"><Plus className="h-4 w-4" />{uiText(" 新增下一個角色")}</button>
            {roles.length >= MAX_ROLES_PER_REQUEST ? <p className="mt-2 text-center text-xs text-slate-500">{uiText("每次最多提交 10 個角色；如需更多，可另外提交一批。")}</p> : null}

            <section className="mt-7 rounded-3xl border border-slate-200 bg-white p-6 sm:p-7">
              <div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-indigo-600" /><div><h2 className="text-sm font-black text-slate-900">{uiText("提交本次需求")}</h2><p className="mt-1 text-xs leading-5 text-slate-500">{uiText("上傳內容只供本次數字人配置使用，請勿包含學生個人資料或未獲授權內容。")}</p></div></div>
              {error ? <div role="alert" className="mt-4 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-bold text-rose-700">{uiError(error)}</div> : null}
              <button type="button" onClick={submit} disabled={!allConfirmed || !contactComplete || submitting} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-indigo-600 px-6 py-4 text-sm font-black text-white shadow-[0_8px_20px_-8px_rgba(79,70,229,.5)] transition enabled:hover:bg-indigo-700 disabled:cursor-not-allowed disabled:bg-[#c7cbe0]">{submitting ? <><LoaderCircle className="h-4 w-4 animate-spin" />{uiText(" 正在提交…")}</> : uiText('提交本次需求')}</button>
              {!allConfirmed ? <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-center text-xs font-bold text-amber-800">{uiText("請先為每個角色生成 AI 設定草案，並完成確認後再提交。")}</div> : !contactComplete ? <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-center text-xs font-bold text-amber-800">{uiText("請先填妥學校及聯絡資料。")}</div> : null}
            </section>
          </>
        ) : null}
      </div>
...
const SectionTitle: React.FC<{ number: string; title: string; subtitle: string }> = ({ number, title, subtitle }) => (
  <div className="flex items-start gap-3"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-indigo-600 text-xs font-black text-white">{number}</span><div><h2 className="text-base font-black text-slate-900">{uiText(title)}</h2><p className="mt-1 text-xs text-slate-400">{uiText(subtitle)}</p></div></div>
);
  ...
  return (
    <motion.section layout initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.98 }} className="mt-5 overflow-hidden rounded-3xl border border-slate-200 bg-white">
      <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5 sm:px-7">
        <div className="flex items-center gap-3"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-indigo-50 text-xs font-black text-indigo-600">{String(index + 1).padStart(2, '0')}</span><div><div className="flex items-center gap-2"><h3 className="text-sm font-black text-slate-900">{role.name || uiTemplate("角色 {0}", index + 1)}</h3>{role.status === 'confirmed' ? <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-black text-emerald-700"><Check className="h-3 w-3" />{uiText(" 已確認設定")}</span> : null}</div><p className="mt-0.5 text-[11px] text-slate-400">{locked ? uiText('欄位已鎖定，返回修改後可再次編輯。') : uiText('填妥必填欄位後生成 AI 草案。')}</p></div></div>
        {removable && !locked ? <button type="button" onClick={onRemove} className="inline-flex items-center gap-1 rounded-lg px-2 py-1.5 text-xs font-bold text-slate-400 hover:bg-rose-50 hover:text-rose-600"><Trash2 className="h-3.5 w-3.5" />{uiText(" 移除")}</button> : null}
      </div>

      <fieldset disabled={locked} className="p-6 sm:p-7">
        <label><FieldLabel required>{uiText("數字人名稱／角色主題")}</FieldLabel><input value={role.name} onChange={(event) => updateRole(role.id, { name: event.target.value })} className={inputClass} placeholder={uiText("例如：蘇軾／英文對話大使 Sophie／校園吉祥物")} /></label>
        <div className="mt-4 grid gap-4 sm:grid-cols-[1fr_1fr_1.25fr]">
          <label><FieldLabel required>{uiText("預計使用班級")}</FieldLabel><input value={role.classInfo} onChange={(event) => updateRole(role.id, { classInfo: event.target.value })} className={inputClass} placeholder={uiText("例如：4A、4B")} /></label>
          <label><FieldLabel required>{uiText("學生人數（約數）")}</FieldLabel><input inputMode="numeric" value={role.studentCount} onChange={(event) => updateRole(role.id, { studentCount: event.target.value })} className={inputClass} placeholder={uiText("例如：60")} /></label>
        ...
        <AnimatePresence>{role.styles.includes('mascot') ? <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden"><div className="mt-4"><FieldLabel>{uiText("上傳吉祥物／校徽圖片")}</FieldLabel><label className="flex h-36 w-full max-w-sm cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-indigo-200 bg-indigo-50/40 text-center"><input type="file" accept=".png,.jpg,.jpeg,.webp" multiple className="hidden" onChange={(event) => updateRole(role.id, { referenceFiles: Array.from(event.target.files || []) })} /><ImagePlus className="h-5 w-5 text-indigo-500" /><span className="mt-2 text-xs font-bold text-indigo-600">{uiText("點擊上傳 JPG／PNG")}</span></label><FileChips files={role.referenceFiles} onRemove={(fileIndex) => updateRole(role.id, { referenceFiles: role.referenceFiles.filter((_, index) => index !== fileIndex) })} /></div></motion.div> : null}</AnimatePresence>

        <div className="mt-6"><FieldLabel optional>{uiText("教學文檔／單元教材")}</FieldLabel><label className="flex cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-indigo-200 bg-indigo-50/30 px-5 py-6 text-center transition hover:bg-indigo-50"><input type="file" accept=".pdf,.doc,.docx,.ppt,.pptx,.txt" multiple className="hidden" onChange={(event) => updateRole(role.id, { materialFiles: Array.from(event.target.files || []) })} /><UploadCloud className="h-5 w-5 text-indigo-500" /><span className="mt-2 text-xs font-black text-indigo-600">{uiText("點擊或拖入教材")}</span><span className="mt-1 max-w-lg text-[11px] leading-5 text-slate-400">{uiText("支援 PDF、Word、PPT；單一檔案上限 20MB，全部檔案合計上限 60MB。")}</span></label><FileChips files={role.materialFiles} onRemove={(fileIndex) => updateRole(role.id, { materialFiles: role.materialFiles.filter((_, index) => index !== fileIndex) })} /><textarea rows={3} value={role.materialText} onChange={(event) => updateRole(role.id, { materialText: event.target.value })} className={`${inputClass} mt-3 resize-y`} placeholder={uiText("或直接貼上課文、單元內容或知識點文字…")} /></div>

        <label className="mt-6 block"><FieldLabel required>{uiText("角色背景資料")}</FieldLabel><textarea rows={4} value={role.background} onChange={(event) => updateRole(role.id, { background: event.target.value })} className={`${inputClass} resize-y`} placeholder={uiText("請簡述角色身份、背景故事、性格取向或需參考的人設資料")} /></label>
        <label className="mt-5 block"><FieldLabel optional>{uiText("補充需求")}</FieldLabel><textarea rows={3} value={role.notes} onChange={(event) => updateRole(role.id, { notes: event.target.value })} className={`${inputClass} resize-y`} placeholder={uiText("例如：希望多用廣東話反問句引導思考、採用半身視角…")} /></label>
      </fieldset>

      <div className="border-t border-slate-100 px-6 py-5 sm:px-7">
        {role.status === 'editing' ? <button type="button" onClick={onGenerate} disabled={!ready} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-indigo-600 px-5 py-3.5 text-sm font-black text-white transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:bg-[#c7cbe0]"><Sparkles className="h-4 w-4" />{uiText(" AI 生成詳細角色設定")}</button> : null}
        {role.status === 'generating' ? <div className="flex items-center justify-center gap-3 rounded-2xl bg-violet-50 px-5 py-4 text-sm font-black text-violet-700"><LoaderCircle className="h-4 w-4 animate-spin" />{uiText(" AI 正在整理角色定位、語氣與教學範圍…")}</div> : null}
        {(role.status === 'proposal' || role.status === 'confirmed') ? <div className="rounded-2xl border border-violet-200 bg-[#faf9ff] p-5"><div className="flex items-center gap-2 text-sm font-black text-violet-700"><Sparkles className="h-4 w-4" />{uiText(" AI 角色設定草案")}</div><p className="mt-3 whitespace-pre-wrap text-sm leading-7 text-indigo-900">{role.proposalText}</p><div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">{role.status === 'proposal' ? <><button type="button" onClick={() => updateRole(role.id, { status: 'editing' })} className="rounded-xl border border-violet-200 bg-white px-4 py-2.5 text-xs font-black text-violet-700 hover:bg-violet-50">{uiText("返回修改")}</button><button type="button" onClick={() => updateRole(role.id, { status: 'confirmed' })} className="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-5 py-2.5 text-xs font-black text-white hover:bg-indigo-700"><Check className="h-4 w-4" />{uiText(" 確認角色設定")}</button></> : <button type="button" onClick={() => updateRole(role.id, { status: 'editing', proposalText: '' })} className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-black text-slate-600 hover:bg-slate-50">{uiText("重新編輯設定")}</button>}</div></div> : null}
        {role.status === 'editing' && !ready ? <p className="mt-2 text-center text-[11px] font-semibold text-slate-400">{uiText("請先填寫角色名稱、班級、學生人數及背景資料。")}</p> : null}
      </div>
    </motion.section>
  );
};
