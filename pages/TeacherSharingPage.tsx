...
export const TeacherSharingPage: React.FC = () => {
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <h1 className="text-3xl font-black tracking-tight text-slate-900">{uiText("學生與 Bot 分享")}</h1>
      <p className="mt-2 text-sm text-slate-500">{uiText("先加入學生帳戶，再選擇要分享給每位學生的 AI Bot。")}</p>
      {(message || error) && <div className={`mt-5 rounded-2xl border px-4 py-3 text-sm ${error ? "border-rose-200 bg-rose-50 text-rose-700" : "border-emerald-200 bg-emerald-50 text-emerald-700"}`}>{uiError(error) || uiText(message)}</div>}

      <section className="mt-6 rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex items-center gap-2 text-lg font-bold"><Users className="h-5 w-5 text-indigo-600" />{uiText("我的學生")}</div>
        <div className="mt-4 flex flex-col gap-3 sm:flex-row">
          <input value={studentEmail} onChange={(e) => setStudentEmail(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") void addStudent(); }} placeholder={uiText("輸入學生帳戶電郵")} className="min-w-0 flex-1 rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-indigo-300 focus:ring-4 focus:ring-indigo-100" />
          <button onClick={() => void addStudent()} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-indigo-600 px-5 py-3 text-sm font-bold text-white hover:bg-indigo-700"><Plus className="h-4 w-4" />{uiText("加入學生")}</button>
        </div>
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          {!loading && !students.length ? <p className="text-sm text-slate-400">{uiText("尚未加入學生。")}</p> : null}
        </div>
      </section>
      ...
      <section className="mt-6 rounded-[30px] border border-slate-200 bg-white shadow-[0_20px_60px_rgba(15,23,42,0.06)]">
        <div className="border-b border-slate-100 px-5 py-4 sm:px-6">
          <div className="flex items-center gap-2 text-sm font-black text-slate-700">
            <Bot className="h-4.5 w-4.5 text-indigo-600" />{uiText("選擇 Bot 與學生")}</div>
          <p className="mt-1 text-[11px] text-slate-500">{uiText("先選 Bot，再在右側快速配置可共享的學生名單。")}</p>
        </div>
          ...
          <div className="grid gap-0 lg:grid-cols-[320px_minmax(0,1fr)]">
            <div className="border-b border-slate-100 bg-slate-50/70 p-4 lg:border-b-0 lg:border-r">
              <div className="mb-3 flex items-center justify-between">
                ...
                <div className="rounded-full bg-white px-2.5 py-1 text-[11px] font-bold text-slate-500">{bots.length}{uiText(" 個")}</div>
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-0.5 text-[11px] text-slate-500">{uiText(bot.subject) || uiText("未分類")}</div>
                      </div>
                      <div className="flex shrink-0 flex-col items-end gap-1">
                        <span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-black text-slate-600">{count}{uiText(" 位學生")}</span>
              ...
              {selectedBot ? (
                <div className="flex h-full min-h-[520px] flex-col rounded-[26px] border border-slate-100 bg-gradient-to-b from-white to-slate-50/60 p-4 sm:p-5">
                  <div className="flex flex-col gap-3 border-b border-slate-100 pb-4 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="mt-0.5 text-xs text-slate-500">{uiText(selectedBot.subject) || uiText("未分類")}{uiText(" · 已選 ")}{selectedStudentIds.size}{uiText(" 位學生")}</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="rounded-full bg-white px-3 py-1.5 text-[11px] font-black text-slate-500 shadow-sm ring-1 ring-slate-200">
                        <UserRound className="mr-1 inline h-3.5 w-3.5" />{uiText("點選學生卡片切換")}</div>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 py-2.5 shadow-sm">
                    ...
                    />
                    {studentQuery ? (
                      <button type="button" onClick={() => setStudentQuery("")} className="text-xs font-bold text-slate-400 hover:text-slate-700">{uiText("清除")}</button>
                    ) : null}
                  </div>

                  <div className="mt-4 flex items-center justify-between text-[11px] font-black text-slate-500">
                    <div className="inline-flex items-center gap-1.5">
                      <Filter className="h-3.5 w-3.5" />{uiText("學生名單")}</div>
                    <div className="rounded-full bg-indigo-50 px-2.5 py-1 text-indigo-600">
                      {visibleStudents.length}{uiText(" 位")}</div>
                  </div>
                  ...
                  <div className="mt-auto border-t border-slate-100 pt-4">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      <div className="text-[11px] text-slate-500">{uiText("已選學生數：")}<span className="font-black text-slate-900">{selectedStudentIds.size}</span>
                      </div>
                      <button
                      ...
                      >
                        {savingBotId === selectedBot.id ? uiText("保存中...") : uiText("保存分享設定")}
                      </button>
                    </div>
                  </div>
                </div>
              ) : (
                <div className="flex h-full min-h-[520px] items-center justify-center rounded-[26px] border border-dashed border-slate-200 bg-white text-sm text-slate-400">{uiText("尚未選擇 Bot。")}</div>
              )}
            </div>
          </div>
        )}
