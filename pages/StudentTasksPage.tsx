...
const JumpButton: React.FC<{ label: string; disabled?: boolean; onClick: () => void }> = ({
  ...
  >
    {disabled ? uiText("正在開啟…") : uiText(label)}
  ...
  return (
    <div className="min-h-screen w-full bg-[var(--bg-app)] text-[var(--text-body)]">
      <div className="mx-auto w-full max-w-3xl px-6 py-8 lg:px-8">
        ...
        <h1 className="mt-6 text-3xl font-black tracking-tight text-[var(--text-main)]">{uiText("📋 今日任務")}</h1>
        <p className="mt-2 text-sm leading-6 text-[var(--text-muted)]">{uiText("老師給你的新消息與待完成挑戰")}</p>
        ...
        {error ? (
          <div className="mt-8 flex items-center justify-between gap-4 rounded-2xl border border-rose-200 bg-rose-50 px-5 py-4 text-sm text-rose-700">
            ...
            <button type="button" onClick={() => void loadTasks()} className="shrink-0 font-bold underline underline-offset-4">{uiText("重新載入")}</button>
          </div>
        ) : null}
        ...
        {!loading && pending.length > 0 ? (
          <section className="mt-8">
            <h2 className="flex items-center gap-2 text-sm font-black text-[var(--text-main)]">
              <span aria-hidden="true">📝</span>{uiText(" 待完成")}<span className="rounded-full bg-[var(--accent-soft)] px-2 py-0.5 text-[11px] font-bold text-[var(--accent-text)]">{pending.length}</span>
            </h2>
            <div className="mt-4 space-y-3">
              {pending.map((item, index) => (
                ...
                >
                  ...
                  <JumpButton
                    label={item.type === "quiz" ? uiText("去做測試") : uiText("查看 Bot")}
        ...
        {!loading && recentShares.length > 0 ? (
          <section className="mt-8">
            <h2 className="flex items-center gap-2 text-sm font-black text-[var(--text-main)]">
              <span aria-hidden="true">🎁</span>{uiText(" 老師分享（最近 3 天）")}</h2>
            <div className="mt-4 space-y-3">
              {recentShares.map((item, index) => (
                ...
                >
                  <div className="flex min-w-0 items-start gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-bold leading-6 text-[var(--text-main)]">
                        {item.teacherName}{uiText(" 分享了機器人「")}{item.botName}{uiText("」給你")}</p>
                      <div className="mt-1.5 flex flex-wrap items-center gap-2 text-xs text-[var(--text-muted)]">
                        <span className="rounded-full bg-[var(--bg-subtle-2)] px-2.5 py-0.5 font-semibold text-[var(--text-body)]">{uiText(item.subject) || uiText("未分類")}</span>
                        <span>{uiText(formatTime(item.sharedAt))}</span>
                      </div>
                    </div>
                  </div>
                  <JumpButton label={uiText("開始對話")} onClick={() => navigateToBot(item.botId)} />
                </motion.div>
              ))}
            </div>
          </section>
