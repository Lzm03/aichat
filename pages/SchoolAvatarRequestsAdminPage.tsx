...
const formatDate = (value: string) => new Intl.DateTimeFormat('zh-HK', {
    ...
    <div className="mx-auto max-w-[1500px] text-slate-900">
      ...
      <div className="mt-5 flex gap-2 overflow-x-auto pb-1">
        {([['all', '全部'], ['new', '新申請'], ['reviewing', '處理中'], ['completed', '已完成']] as const).map(([value, label]) => (
          <button key={value} onClick={() => setFilter(value)} className={`whitespace-nowrap rounded-full px-4 py-2 text-xs font-black transition ${filter === value ? 'bg-[#1b365d] text-white' : 'bg-white text-slate-500 hover:bg-slate-100'}`}>{label}</button>
        ))}
      </div>

      {error ? <div role="alert" className="mt-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-bold text-red-700">{error}</div> : null}
      ...
      <div className="mt-5 grid min-h-[640px] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm lg:grid-cols-[360px_minmax(0,1fr)]">
        ...
        <section className="min-w-0 p-5 sm:p-7 lg:max-h-[640px] lg:overflow-y-auto">
          {detailLoading ? <div className="flex h-64 items-center justify-center text-slate-400"><LoaderCircle className="h-7 w-7 animate-spin" /></div> : !detail ? (
            <div className="flex h-64 flex-col items-center justify-center text-center"><Inbox className="h-10 w-10 text-slate-300" /><p className="mt-3 text-sm font-bold text-slate-500">請選擇一份申請</p></div>
            ...
            <div className="flex flex-col gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-start sm:justify-between">
              ...
              <select value={detail.request.status} onChange={(event) => void updateStatus(event.target.value as RequestSummary['status'])} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-700 outline-none focus:border-[#1b365d]">
                <option value="new">新申請</option><option value="reviewing">處理中</option><option value="completed">已完成</option>
              </select>
            </div>

            <div className="grid gap-x-8 gap-y-4 border-b border-slate-200 py-6 sm:grid-cols-2">
              <div className="flex items-start gap-3"><UserRound className="mt-0.5 h-4 w-4 text-slate-400" /><div><p className="text-xs font-bold text-slate-400">聯絡老師</p><p className="mt-1 text-sm font-bold text-slate-700">{detail.request.teacher_name}</p></div></div>
              <a href={`tel:${detail.request.phone}`} className="flex items-start gap-3"><Phone className="mt-0.5 h-4 w-4 text-slate-400" /><div><p className="text-xs font-bold text-slate-400">電話／WhatsApp</p><p className="mt-1 text-sm font-bold text-[#1b365d]">{detail.request.phone}</p></div></a>
              <a href={`mailto:${detail.request.email}`} className="flex items-start gap-3"><Mail className="mt-0.5 h-4 w-4 text-slate-400" /><div><p className="text-xs font-bold text-slate-400">電郵</p><p className="mt-1 break-all text-sm font-bold text-[#1b365d]">{detail.request.email}</p></div></a>
              <div className="flex items-start gap-3"><CalendarDays className="mt-0.5 h-4 w-4 text-slate-400" /><div><p className="text-xs font-bold text-slate-400">提交時間</p><p className="mt-1 text-sm font-bold text-slate-700">{formatDate(detail.request.created_at)}</p></div></div>
            </div>

            <div className="py-6"><h3 className="text-sm font-black text-slate-900">角色需求 · {detail.roles.length}</h3><div className="mt-4 space-y-5">
              {detail.roles.map((role) => {
                ...
                return <div key={role.id} className="border-l-2 border-blue-100 pl-4 sm:pl-5"><div className="flex items-center gap-3"><span className="text-xs font-black text-[#e63946]">{String(role.role_index + 1).padStart(2, '0')}</span><h4 className="font-black text-slate-800">{role.name}</h4></div>{tags.length ? <div className="mt-3 flex flex-wrap gap-2">{tags.map((tag) => <span key={tag} className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">{tag}</span>)}</div> : null}{role.material_text ? <div className="mt-4"><p className="text-xs font-bold text-slate-400">教材文字</p><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-slate-600">{role.material_text}</p></div> : null}{role.notes ? <div className="mt-4"><p className="text-xs font-bold text-slate-400">補充需求</p><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-slate-600">{role.notes}</p></div> : null}{files.length ? <div className="mt-4 space-y-2">{files.map((file) => <button key={file.id} onClick={() => void downloadFile(file)} className="flex w-full items-center gap-3 rounded-xl border border-slate-200 px-3 py-3 text-left transition hover:border-blue-200 hover:bg-blue-50/40"><FileText className="h-4 w-4 shrink-0 text-[#1b365d]" /><span className="min-w-0 flex-1"><strong className="block truncate text-sm text-slate-700">{file.original_name}</strong><span className="text-xs text-slate-400">{file.kind === 'reference' ? '角色參考圖' : '教材'} · {formatBytes(file.size_bytes)}</span></span>{downloadingId === file.id ? <LoaderCircle className="h-4 w-4 animate-spin text-slate-400" /> : <Download className="h-4 w-4 text-slate-400" />}</button>)}</div> : null}</div>;
              })}
            </div></div>

            {detail.request.status === 'completed' ? <div className="flex items-center gap-2 border-t border-slate-200 pt-5 text-sm font-bold text-emerald-700"><CheckCircle2 className="h-5 w-5" /> 此申請已標記為完成</div> : null}
