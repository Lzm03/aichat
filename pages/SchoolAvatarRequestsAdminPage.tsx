...
import { CalendarDays, Check, CheckCircle2, ChevronRight, Copy, Download, FileText, Inbox, LoaderCircle, Mail, Phone, RefreshCw, School, UserRound } from 'lucide-react';
...
export const SchoolAvatarRequestsAdminPage: React.FC = () => {
  ...
  const [copiedRoleId, setCopiedRoleId] = useState('');

  const copyProposal = async (role: RequestRole) => {
    try {
      await navigator.clipboard.writeText(role.proposal_text);
      setCopiedRoleId(role.id);
      window.setTimeout(() => setCopiedRoleId((current) => (current === role.id ? '' : current)), 2000);
    } catch {
      setCopiedRoleId('');
    }
  };
          ...
          {detailLoading ? <div className="flex h-64 items-center justify-center text-slate-400"><LoaderCircle className="h-7 w-7 animate-spin" /></div> : !detail ? (
            ...
            <div className="py-6"><h3 className="text-sm font-black text-slate-900">{uiText("角色需求 · ")}{detail.roles.length}</h3><div className="mt-4 space-y-5">
              {detail.roles.map((role) => {
                ...
                return <div key={role.id} className="border-l-2 border-blue-100 pl-4 sm:pl-5"><div className="flex items-center gap-3"><span className="text-xs font-black text-[#e63946]">{String(role.role_index + 1).padStart(2, '0')}</span><h4 className="font-black text-slate-800">{role.name}</h4></div>{tags.length ? <div className="mt-3 flex flex-wrap gap-2">{tags.map((tag) => <span key={tag} className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">{uiText(tag)}</span>)}</div> : null}{role.material_text ? <div className="mt-4"><p className="text-xs font-bold text-slate-400">{uiText("教材文字")}</p><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-slate-600">{role.material_text}</p></div> : null}{role.proposal_text ? <div className="mt-4 rounded-xl border border-violet-200 bg-[#faf9ff] p-4"><div className="flex items-center justify-between gap-3"><p className="text-xs font-bold text-violet-700">{uiText("角色設定書（可直接存入 Bot）")}</p><button type="button" onClick={() => void copyProposal(role)} className="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-violet-200 bg-white px-2.5 py-1.5 text-xs font-black text-violet-700 transition hover:bg-violet-50">{copiedRoleId === role.id ? <><Check className="h-3.5 w-3.5" />{uiText("已複製")}</> : <><Copy className="h-3.5 w-3.5" />{uiText("複製")}</>}</button></div><pre className="mt-3 whitespace-pre-wrap break-words font-sans text-sm leading-6 text-indigo-900">{role.proposal_text}</pre></div> : null}{role.notes ? <div className="mt-4"><p className="text-xs font-bold text-slate-400">{uiText("申請備註")}</p><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-slate-600">{role.notes}</p></div> : null}{files.length ? <div className="mt-4 space-y-2">{files.map((file) => <button key={file.id} onClick={() => void downloadFile(file)} className="flex w-full items-center gap-3 rounded-xl border border-slate-200 px-3 py-3 text-left transition hover:border-blue-200 hover:bg-blue-50/40"><FileText className="h-4 w-4 shrink-0 text-[#1b365d]" /><span className="min-w-0 flex-1"><strong className="block truncate text-sm text-slate-700">{file.original_name}</strong><span className="text-xs text-slate-400">{file.kind === 'reference' ? uiText('角色參考圖') : uiText('教材')} · {formatBytes(file.size_bytes)}</span></span>{downloadingId === file.id ? <LoaderCircle className="h-4 w-4 animate-spin text-slate-400" /> : <Download className="h-4 w-4 text-slate-400" />}</button>)}</div> : null}</div>;
              })}
            </div></div>
